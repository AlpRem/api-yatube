...
from .serializers import PostSerializer, GroupSerializer, CommetSerializer
...
def api_comment_detail(request, post_id, comment_id):
    ...
    if comment.author != request.user:
        return Response(status=status.HTTP_403_FORBIDDEN)
    if request.method in ('PUT', 'PATCH'):
        serializer = CommetSerializer(comment, data=request.data, partial=request.method == 'PATCH')
        if serializer.is_valid():
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    comment.delete()
